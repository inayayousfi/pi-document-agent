...
import { runHttpMode } from "./modes/http/http-mode.ts";
...
function toPrintOutputMode(appMode: AppMode): Exclude<Mode, "rpc" | "http"> {
...
export async function main(args: string[], options?: MainOptions) {
	...
	// http mode keeps stdout for its own console.log startup message
	const shouldTakeOverStdout =
		appMode !== "interactive" && appMode !== "http" && !isPlainRuntimeMetadataCommand(parsed);
	...
	if (appMode === "http") {
		printTimings();
		await runHttpMode(runtime, { port: parsed.port ?? 3000 });
	} else if (appMode === "rpc") {
		printTimings();
